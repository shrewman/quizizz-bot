...
async function extractTextFromElement(page, selector) {
  ...
  const childTexts = await Promise.all(
    children.map((child) =>
      extractTextFromElement(page, selector + " > " + child.tagName),
    ),
  );
...
function getCorrectOptionIndexes(question, answers, optionsText) {
  ...
  for (let i = 0; i < answers.length; i++) {
    ...
    const optionIndex = optionsText.findIndex((text) =>
      card.answer.includes(text),
    );
    if (optionIndex >= 0) {
...
async function handleMultipleChoiceQuestion(
  page,
  question,
  answers,
  probability,
...
prompt.question("Input room code: ", (answer) => {
  ...
  prompt.question(
    "Input the probability of choosing correct answer (default: 0.9): ",
    (answer) => {
      probability = answer.trim() || "0.9";
      prompt.question(
        "Input the estimated time spent on each question in seconds (default: 10): ",
        (answer) => {
          timeOnQuestion = answer.trim() * 1000 || 10000;
          prompt.question(
            "Input your name, that will be showed up in quiz: ",
            (answer) => {
              name = answer;
              prompt.close();
            },
